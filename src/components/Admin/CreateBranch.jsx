...
import { Building2, MapPin, User, Phone, Globe, CheckCircle2 } from 'lucide-react';
        ...
        <form onSubmit={e => { e.preventDefault(); setSubmitted(true); }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 14 }}>
              <button type="submit" style={{ padding: '13px 36px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', color: '#fff', fontWeight: 800, fontSize: 15, cursor: 'pointer', boxShadow: '0 4px 16px rgba(79,70,229,0.4)' }}>
                Create Branch
              </button>
              <button type="button" onClick={() => setForm({ name: '', type: 'Branch', address: '', city: '', state: '', pincode: '', manager: '', phone: '', email: '', capacity: '' })} style={{ padding: '13px 28px', borderRadius: 14, border: isDark ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,0,0,0.1)', background: 'transparent', color: isDark ? '#94a3b8' : '#64748b', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>
                Reset
              </button>
            </div>
          </div>
