...
// Timeout helper for Prisma calls - 1200ms fast fallback
...
export async function getVisitors() {
  ...
  // Merge DB visitors and File visitors so NO new visitor is missed
...
export async function createVisitor(data) {
  ...
  // Always sync to local file storage FIRST for 100% availability
  const visitors = readJsonFile('visitors.json', []);
  visitors.unshift(newRecord);
  writeJsonFile('visitors.json', visitors);

  // Try Prisma DB insertion in background/with timeout
  try {
    let tenant = await withTimeout(prisma.tenant.findFirst(), 1200).catch(() => null);
    if (!tenant) {
      tenant = await withTimeout(prisma.tenant.create({ data: { name: 'Default Tenant', domain: 'default.com' } }), 1200).catch(() => null);
    }
    ...
    if (tenant) {
      ...
      }), 2000).catch(err => console.warn("Prisma visitor creation skipped/timed out:", err.message));
    }
  } catch (err) {
    console.warn("Prisma save failed, stored in local JSON:", err.message);
  }
...
export async function updateVisitor(id, updates) {
  ...
  // Update local file storage first
  const visitors = readJsonFile('visitors.json', []);
  ...
  // Update Prisma DB in background
  try {
...
export async function getVisitorById(id) {
  ...
  // Check local file storage first for fast response
  const fileVisitors = readJsonFile('visitors.json', []);
